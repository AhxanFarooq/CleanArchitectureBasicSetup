...
namespace Persistance.Repositories
{
    public class QuotationRepository : BaseRepository<Quotation>, IQuotationRepository
    {
        ...
        public Task<string> GetAutoCode( CancellationToken cancellationToken)
        {
            return _context.Quotations.OrderBy(x => x.Code)
                              .Select(x => x.Code) // Select only the Code column
        ...
        {
            var data = await _context.Set<Quotation>().Include(x => x.QuotationItems).Include(x => x.Contact).ToListAsync(cancellationToken);
